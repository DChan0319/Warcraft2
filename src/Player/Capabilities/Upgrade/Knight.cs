...
{
	public class PlayerCapabilityBuildKnight : PlayerCapability
	{
		private string UnitName;

		public PlayerCapabilityBuildKnight(string unitName) : base($"Build{unitName}", TargetType.None)
		{
			UnitName = unitName;
		}
		...
		{
			if (actor.Data.Type == AssetType.Blacksmith)
			{
			...
			public override bool IncrementStep()
			{
				...
				if (CurrentStep >= TotalSteps)
				{
					if (Actor.Data.Type == AssetType.Blacksmith)
					{
						...
						var knight = PlayerData.AssetDatas["Knight"];

						barracks.AddCapability(AssetCapabilityType.BuildKnight);
						barracks.AddCapability(AssetCapabilityType.BuildFootman);

						// Upgrade all footmen
						foreach (var asset in PlayerData.Assets)
						{
							if (asset.Data.Type == AssetType.Footman)
							{
								var addHealth = knight.Health - asset.Data.Health;
								asset.Data = knight;
		...
		public class Registrant
		{
			public Registrant()
			{
				Register(new PlayerCapabilityBuildKnight("Knight"));
			}
		}
	}
}